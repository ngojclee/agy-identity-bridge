...
func publicStatusPage(diagnostics providerDiagnostics) string {
	return dashboardHTML(diagnostics)
}

func dashboardHTML(diagnostics providerDiagnostics) string {
	...
	state, stateTone := dashboardRouteState(public)
	mirrored := "Not matched"
	if public.MirroredProvider != "" {
		mirrored = public.MirroredProvider
	}
	executor := "Off"
	if public.ExecutorEnabled {
		executor = public.ExecutorProvider
	}
	prefix := strings.Join(public.ActivePrefixes, ", ")
	if prefix == "" {
		prefix = "none"
	}
	originalProvider := "Not matched"
	if public.MirroredProvider != "" {
		if public.ProviderOriginalEnabled {
			originalProvider = "Enabled"
		} else {
			originalProvider = "Disabled"
		}
	}
	secret := dashboardCheck(public.HMACSecretConfigured,
		"HMAC secret configured",
		"No HMAC secret configured")
	authReady := dashboardCheck(public.ExecutorAuthEnsured,
		"ln.Antigravity auth ready",
		"ln.Antigravity auth not ready")
	modelState := dashboardCheck(public.ModelsServed,
		"Plugin models published",
		"Plugin models withheld")
	upstream := "No agy2api response yet"
	upstreamTone := "muted"
	if public.LastExecutorStatus != 0 {
		upstream = fmt.Sprintf("agy2api HTTP %d", public.LastExecutorStatus)
		upstreamTone = dashboardStatusTone(public.LastExecutorStatus)
	}

	checks := secret + authReady + modelState
	var events strings.Builder
	if len(public.RecentEvents) == 0 {
		events.WriteString(`<div class="empty">No runtime events since the plugin loaded.</div>`)
	} else {
		for _, event := range public.RecentEvents {
...
.page-head{margin-bottom:16px}.subtitle{color:var(--ink-2);font-size:13px}.button{appearance:none;border:1px solid var(--line-2);background:var(--panel);color:var(--ink-2);font:inherit;font-size:12px;font-weight:600;padding:6px 10px;border-radius:6px;cursor:pointer}.button:hover{border-color:var(--line-2);background:#fff;color:var(--ink)}
...
<body>
<main>
<div class="page-head"><div class="row between"><div><h1>AGY Identity Bridge</h1><div class="subtitle">Antigravity bridge diagnostics and runtime status</div></div><button class="button" onclick="location.reload()">Refresh</button></div></div>
<section class="hero">
...
<details><summary>Redacted JSON</summary><pre>%s</pre></details>
		...
		dashboardPublishedModelCount(public),
		public.RuntimeAuthCount,
		...
		html.EscapeString(public.HMACSecretSource),
		warningBlock.String(),
